...
from scraping.models import Category,Language
User = get_user_model()
...
class UserUpdateForm(forms.Form):
    ...
    required=True, widget=forms.Select(attrs={'class':'form-control'}), label='Категория')

    language = forms.ModelChoiceField(queryset=Language.objects.all(), to_field_name='slug',
    required=True, widget=forms.Select(attrs={'class':'form-control'}), label='Язык программирования')

    send_email = forms.BooleanField(required=False, widget=forms.CheckboxInput,label='Получать рассылку?')

    class Meta:
        model = User
        fields = ('category','language','send_email')

class ContactForm(forms.Form):
    category = forms.CharField(required=True, widget=forms.TextInput(attrs={'class':'form-control'}), label='Категория')
