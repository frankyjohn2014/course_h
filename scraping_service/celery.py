...
@periodic_task(run_every=(crontab(minute='*/1')), name="check_mp4_404")
def course_404():
    from scraping.models import Post_video,Post
    headers = {
    ...
    def parse_and_save(status):
        if status == 418:
            ...
            def save_data(): 
                ...
                while True:
                    try:
                        Post_video.objects.update_or_create(
                            title = title_links[d],
                            videos = videos[d],
                            posts_id = posts[d]
                        )
            ...
            print('No need parsing and save, statuscode:', status)
    def get_models_and_check_status():
        
        # Post_video1 = apps.get_model(app_label='scraping', model_name='Post_video')
        # Post1 = apps.get_model(app_label='scraping', model_name='Post')
        response = requests.post('https://coursehunter.net/api/history', headers=headers, data=data)
        try:
            instance_obj = Post_video.objects.get(id=1)
            url = instance_obj.videos
            ...
            status = 418
            print('STATUSCODE', status)
        parse_and_save(status)


    get_models_and_check_status()
